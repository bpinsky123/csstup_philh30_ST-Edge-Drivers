...
local function map_fan_4_speed_to_switch_level (speed)
  if speed == fan_speed_helper.fan_speed.OFF then
    return fan_speed_helper.levels_for_4_speed.OFF -- off
  elseif speed == fan_speed_helper.fan_speed.LOW then
    return fan_speed_helper.levels_for_4_speed.LOW -- low
  elseif speed == fan_speed_helper.fan_speed.MEDIUM then
    return fan_speed_helper.levels_for_4_speed.MEDIUM -- medium
  elseif speed == fan_speed_helper.fan_speed.HIGH then
    return fan_speed_helper.levels_for_4_speed.HIGH -- high
  elseif speed == fan_speed_helper.fan_speed.MAX then
    return fan_speed_helper.levels_for_4_speed.MAX -- max
  else
    log.error (string.format("4 speed fan driver: invalid speed: %d", speed))
  end
end

local function map_switch_level_to_fan_4_speed (level)
  if (level == fan_speed_helper.levels_for_4_speed.OFF) then
...
local function is_fan_4_speed(opts, driver, device, ...)
